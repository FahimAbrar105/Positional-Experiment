"""Moral Foundations Vignettes (MFV), Clifford, Iyengar, Cabeza & Sinnott-Armstrong
(2015). 132 short vignettes across 7 categories (6 moral foundations + a
non-moral "Social Norms" control category), each rated for moral wrongness.
"""
...
_FAIRNESS = [
    ...
    "You see a boy skipping to the front of the line because his friend is an employee.",
...
_AUTHORITY = [
    ...
    "You see an employee trying to undermine all of her boss' ideas in front of others.",
...
_LIBERTY = [
    ...
    "You see a public leader on TV trying to ban the wearing of hooded sweatshirts.",
    "You see a mother forcing her daughter to enroll as a pre-med student in college.",
    "You see a father requiring his son to take up the family restaurant business.",
...
_SOCIAL_NORMS = [
    ...
    "You see a man staying inside his home with the shades drawn on a rare sunny day.",
    "You see a woman drinking her entire cup of coffee using a stirring spoon.",
    "You see a woman eating dessert before her main entree arrives on the table.",
    "You see a woman continuing to wear a large sun hat inside her apartment complex.",
    ...
    "You see a woman using a fork to eat a bowl of vanilla ice cream and marshmallows.",
    "You see a man putting ketchup all over his chicken Caesar salad while at lunch.",
    "You see a man wearing clothes that are clearly several sizes too big for his body.",
    ...
    "You see a woman watching a game on television in black-and-white instead of color.",
    ...
    "You see a woman answering a phone call with the word “goodbye” instead of “hello.”",
]

_CATEGORIES = [
    ("Care", _CARE), ("Fairness", _FAIRNESS), ("Loyalty", _LOYALTY),
    ("Authority", _AUTHORITY), ("Sanctity", _SANCTITY), ("Liberty", _LIBERTY),
    ("Social Norms", _SOCIAL_NORMS),
]

_ITEMS = [
    Item(item_id=f"mfv_{block.lower().replace(' ', '')}{i}", text=text, block=block, reverse=False)
    for block, texts in _CATEGORIES
    for i, text in enumerate(texts, start=1)
]

_BLOCKS = [name for name, _ in _CATEGORIES]

_ANCHORS = {
    1: "not at all morally wrong",
    2: "not very morally wrong",
    3: "slightly morally wrong",
    4: "moderately morally wrong",
    5: "morally wrong",
    6: "very morally wrong",
    7: "extremely morally wrong",
}

_INSTRUCTIONS = (
    "You will be shown a numbered list of short scenarios. For each one, rate how "
    "morally wrong the behavior described is, using your own judgement."
)


class MFVInstrument(LikertInstrument):
    def __init__(self):
        super().__init__(
            name="Moral Foundations Vignettes (MFV)",
            items=_ITEMS,
            blocks=_BLOCKS,
            scale_min=1,
            scale_max=7,
